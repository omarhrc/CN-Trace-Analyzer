...
from parsing.nas_lte import nas_lte_req_regex, s1ap_message_type_regex, nas_lte_message_type_regex
...
color_sip_req = '#a3a3a3'
color_sip_rsp = '#f6f6f6'
color_nas_lte_req = '#7cc97c'
color_nas_lte_rsp = '#b6dbb6'
...
def packet_to_str(packet: PacketDescription, simple_diagrams=False, force_show_frames='', show_timestamp=False) \
    ...
    if 'NGAP' in protocol:
        ...
        # Some customized filtering based on what we have seen
        rsp_match = sip_rsp_regex.search(packet.msg_description)
        req_match = sip_req_regex.search(packet.msg_description)
        if rsp_match is not None:
            note_color = ' {0}'.format(color_sip_rsp)
            protocol = '{0} {1} rsp.'.format(protocol, rsp_match.group(1))
        elif req_match is not None:
            note_color = ' {0}'.format(color_sip_req)
            protocol = protocol + ' req.'
        else:
            note_color = ' {0}'.format(color_sip_req)
            protocol = protocol + ' req. or rsp.'
    ...
    if 'S1AP' in protocol:
        if nas_lte_req_regex.search(packet.msg_description) is not None:
            note_color = ' {0}'.format(color_nas_lte_req)
            protocol = 'NAS-EPS req.'
        else:
            note_color = ' {0}'.format(color_nas_lte_rsp)
            protocol = 'S1AP msg. or NAS-EPS rsp.'

        # Search S1AP messages
        s1ap_matches = s1ap_message_type_regex.finditer(packet.msg_description)
        s1ap_message_types = [s1ap_match.group(1) for s1ap_match in s1ap_matches if s1ap_match is not None]
        if len(s1ap_message_types) > 0:
            s1ap_seen = set()
            s1ap_seen_add = s1ap_seen.add
            s1ap_message_types = ['S1AP {0}'.format(x) for x in s1ap_message_types if
        ...
        # Print msg. type
        joint_s1ap_nas_msg_types = s1ap_message_types + nas_message_types
        if len(joint_s1ap_nas_msg_types) > 0:
            protocol = '{0}'.format(',\\n'.join(joint_s1ap_nas_msg_types))
    ...
    formated_source = '{0}_{1}_{2}'.format(packet.ip_src, packet.port_src, packet.transport_protocol)
    formated_destination = '{0}_{1}_{2}'.format(packet.ip_dst, packet.port_dst, packet.transport_protocol)
    
    packet_str = packet_str + '"{0}" -> "{1}": {2}, {3}\n'.format(formated_source,
                                                                  formated_destination,
    ...
    packet_str = packet_str + '\n'
    return PacketDiagramDescription(packet_str, formated_source, formated_destination, protocol)
