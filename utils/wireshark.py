...
from parsing.nas_lte import parse_lte_nas_proto
from parsing.pfcp import parse_pfcp_proto
from parsing.sip import parse_sip_proto
from utils.plantuml import output_puml

wireshark_folder = 'wireshark'
ip_regex: Pattern[str] = re.compile(r'Src: ([\d\.:]*), Dst: ([\d\.:]*)')
transport_regex: Pattern[str] = re.compile(r'Src Port: (\d+).*Dst Port: (\d+)')
nfs_regex = re.compile(r':path: \/(.*)\/v.*\/.*')
sip_req_regex = re.compile(r'\\n([A-Z]+)\s+(.*?)')
sip_rsp_regex = re.compile(r'(\d{3})(.*)')
...
def call_wireshark_for_one_version(
    ...
    if mode is None:
        tshark_command.extend([
            '-Y',
            '(http2 and (http2.type == 0 || http2.type == 1)) or ngap or nas-5gs or pfcp or gtpv2 or diameter or radius or gtpprime or icmp or icmpv6.type == 134 or sip or sdp or nas-eps',
            '-T',
            'pdml',
            '-J',
...
def import_pdml(file_paths,
    ...
    for idx, packet in enumerate(filtered_root_packets):
        ...
        ' Include now transport protocols'
        transport_protocol = None
        for transport in TRANSPORT_PROTOCOLS:
            try:
                transport_showname = packet.findall(f"proto[@name='{transport}']")[0].attrib['showname']
                transport_protocol = transport
                break
            except:
                pass
        else:
            logging.debug('Transport protocol not recognized in frame {0}'.format(frame_number))
        if transport_protocol:
                transport_match = transport_regex.search(transport_showname)
                port_src = transport_match.group(1)
                port_dst = transport_match.group(2)
        ...
        s1ap_proto = packet.findall("proto[@name='s1ap']")
        if len(s1ap_proto) == 0:
           s1ap_proto = None
        elif len(s1ap_proto) == 1:
        ...
        if s1ap_proto is not None:
            packet_has_s1ap = True
            protocols.append('S1AP')
...
#        destinations.add(ip_src)
        destinations.add((ip_dst, port_dst, transport_protocol))
        destinations.add((ip_src, port_src, transport_protocol))
...
#        destination = packet_description.ip_dst
        destination = (packet_description.ip_dst, 
                        packet_description.port_dst,
                        packet_description.transport_protocol)
        ...
        # NAS_EPS
        if 'S1AP' in packet_description.protocols_str:
            all_destinations[destination].add('S1')

    # Generate participant descriptions for description and add order (key)
    participants = []
    for destination, uses in all_destinations.items():
        formated_destination = '{0}_{1}_{2}'.format(*destination)
        participant_type = 'participant'
        participant_description = ''
        if len(uses) != 0:
            joined_uses = '\\n'.join(uses) + '\\n{0}'.format(formated_destination)
            participants.append(
                (participant_type, '"{0}" as {1}'.format(joined_uses, formated_destination), formated_destination, uses, None))
        else:
            participants.append((participant_type, '"{0}"'.format(formated_destination), formated_destination, uses, None))
    ...
    try:
        ...
        for idx, file in enumerate(pcap_files):
            if os.path.exists(file):
                pdml_file = call_wireshark(wireshark_version, platform, file, http2_ports)
                packet_descriptions = import_pdml(pdml_file, diagrams_to_output='raw', debug=debug)
                packets_df = pd.DataFrame(packet_descriptions,
                                          columns=['ip_src', 'ip_dst', 'port_src', 'port_dst', 'transport_protocol', 'frame_number', 'protocol', 'msg_description',
